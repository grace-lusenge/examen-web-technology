<?php
   ...
   # include('modal.php');
    ...
    vente_produit.quantite) as total,facture.date,(vente_produit.quantite*vente_produit.pvu 
    -vente_produit.quantite*entree_produit.prix_achat) as benefice from vente_produit join 
    entree_produit ON entree_produit.id = vente_produit.id_entree_produit JOIN produit on 
    produit.id = entree_produit.id_produit join facture on facture.id=vente_produit.facture where facture.date>=? AND facture.date<=?");

    $req_vente = $con->prepare("SELECT SUM(vente_produit.quantite*vente_produit.pvu 
    -vente_produit.quantite*entree_produit.prix_achat) as benefice from vente_produit join 
    entree_produit ON entree_produit.id = vente_produit.id_entree_produit JOIN produit on 
    produit.id = entree_produit.id_produit join facture on facture.id=vente_produit.facture where facture.date>=? AND facture.date<=?");
    $req->execute();
    ...
    $req_vente->execute([$date1,$date2]);
?>

<div class="content">
<h2>Rapport de Benefice</h2>
        <form id="apartmentForm" method="post" action="../upload/ventes/upload-rapport-benefice.php">
            <div class="row mt-3">
